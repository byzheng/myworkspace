test_that("render_modified_quarto works end-to-end", {
    ...
    if (Sys.which("quarto") == "") {
        skip("quarto command not available")
    }
...
test_that("render_modified_quarto link among files", {
    ...
    out2 <- render_modified_quarto(root_dir = root, dry_run = FALSE)
    expect_true(file.exists(file.path(root, "_site", "script/a.html")))
    texta <- readLines(file.path(root, "_site", "script/a.html"), warn = FALSE) |> 
        paste(collapse = "\n")
    expect_true(grepl("script/b.html", texta))
    
    # Rerun with changes B
    writeLines(c(
        "# B",
        "",
        "[a](a.qmd)"
    ), file.path(root, "script", "b.qmd"))
    out2 <- render_modified_quarto(root_dir = root, dry_run = FALSE)
    expect_true(file.exists(file.path(root, "_site", "script/b.html")))
    textb <- readLines(file.path(root, "_site", "script/b.html"), warn = FALSE) |> 
        paste(collapse = "\n")
    expect_true(grepl("script/a.html", textb))
